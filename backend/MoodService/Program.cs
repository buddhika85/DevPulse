...
builder.Services.AddValidatorsFromAssemblyContaining<UpdateMoodEntryCommandValidator>();                 // This auto-registers all Fluent Validators (for DTO, command, and query validators) in the assembly for dependency injection.
...
if (app.Environment.IsDevelopment() || app.Environment.IsProduction())
{
    ...
    app.UseSwaggerUI(x =>                                                   // Serves Swagger UI
    {
        ...
        x.SwaggerEndpoint("/swagger/v1/swagger.json", "DevPulse Mood API v1");
    });
}
...
{
    Log.Fatal(ex, "MoodService terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
